<template>
  <div class="repost">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item to="/productList">文章</el-breadcrumb-item>
    ...
    <el-form label-width="120px" :rules="rules" :model="ruleForm" ref="ruleForm" class="demo-ruleForm">
      <div class="form-left">
        <div class="content">
          ...
          <el-form-item label="标题拼音" prop="spell">
            <el-input placeholder="请输入标题拼音" v-model="ruleForm.spell"></el-input>
          </el-form-item>
          <el-form-item label="副标题" prop='subtitle'>
            <el-input placeholder="请输入副标题" v-model="ruleForm.subtitle"></el-input>
          </el-form-item>
          <el-form-item label="产品简介" prop='introduce'>
            <el-input placeholder="请输入产品简介" v-model="ruleForm.introduce"></el-input>
          </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        spell: [
          {required: true, message: '请输入标题拼音', trigger: 'blur'}
        ],
        subtitle: [
  ...
  methods: {
    submitForm (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.httpRequest('/api/product', 'POST', '', options).then(res => {
    ...
    getProdectDetail () {
      this.httpRequest('/api/getProductDetail', 'GET', {id: this.$route.params.id}).then(res => {
        if (res.data.code * 1 === 200) {
          this.ruleForm.title = res.data.data[0].title
          this.ruleForm.subtitle = res.data.data[0].subtitle
          this.ruleForm.spell = res.data.data[0].spell
          this.ruleForm.categories = res.data.data[0].categories
          this.ruleForm.content = res.data.data[0].content
          this.ruleForm.introduce = res.data.data[0].introduce
